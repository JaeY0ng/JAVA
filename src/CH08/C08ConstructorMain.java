...
// 생성자매서드
// 객체 생성 시 필요한 메모리 공간 형성 과 초기 값 부여에 사용되는 메서드
// 객체 생성 시 한번 호출되는 메서드 ( 재사용 불가 )
// 생성자 메서드의 이름은 클래스의 이름과 동일하게 작성하고 반환자료형은 작성하지 않는다
// 생성자 메서드를 므래스 내에 작성하지 않으면 컴파일러에 의해 주입되는 생성자가 있는데
// 이를 디폴트 생성자라고 한다
// 디폴트 생성자는 모든 멤버변수의 초기값을 0 ( or 
