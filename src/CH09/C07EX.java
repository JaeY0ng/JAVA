...
class Profile{
	...
	//ex) new Profile("홍길동","대구","프로그래머","컴퓨터공학") ->가변인자가 받는값 [홍길동,대구,프로그래머,컴퓨터공학]
	...
	boolean isEquals(String str) {
		...
		if(
			this.name.equals(args[0])&&
			this.addr.equals(args[1])&&
			this.job.equals(args[2])&&
			this.major.equals(args[3])
				)
			return true;
